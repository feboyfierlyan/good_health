...
List<RegisPoli> regisPoliFromJson(jsonData) {
  List<RegisPoli> result = List<RegisPoli>.from(
    jsonData.map((item) => RegisPoli.fromJson(item)),
  );
...
Future<List<RegisPoli>> fetchRegisPolis() async {
  ...
  //String route = AppConfig.API_ENDPOINT + "reg_poli/index.php?id_pasien=$idPasien";
  final response = await http.get(
    Uri.parse('${AppConfig.baseUrl}/reg_poli/index.php?id_pasien=$idPasien'),
  );
...
Future regisPoliCreate(RegisPoli regisPoli) async {
  final prefs = await SharedPreferences.getInstance();
  //String route = AppConfig.API_ENDPOINT + "reg_poli/create.php";
  try {
    final response = await http.post(
      Uri.parse('${AppConfig.baseUrl}/reg_poli/create.php'),
...
Future deleteRegisPoli(id) async {
  ...
  final response = await http.get(
    Uri.parse('${AppConfig.baseUrl}/reg_poli/delete.php?id=$id'),
  );
