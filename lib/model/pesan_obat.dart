...
class PesanObat {
  ...
  factory PesanObat.fromJson(Map<String, dynamic> json) {
    return PesanObat(
      // PERBAIKAN: Tambahkan .toString() ke semua bidang
      // yang mungkin berupa angka (terutama ID dan biaya)
      idPesanObat: json['id_pesan_obat'].toString(), // FIX
      idPasien: Pasien.fromJson(json['id_pasien']),
      waktu: json['waktu'],
      alamat: json['alamat'],
      lat: json['lat'].toString(), // FIX
      lng: json['lng'].toString(), // FIX
      listPesanan: json['list_pesanan'],
      totalBiaya: json['total_biaya'].toString(), // FIX
      ket: json['ket'] ?? "",
...
List<PesanObat> pesanObatFromJson(jsonData) {
  List<PesanObat> result = List<PesanObat>.from(
    jsonData.map((item) => PesanObat.fromJson(item)),
  );
...
Future<List<PesanObat>> fetchPesanObats({isSelesai}) async {
  ...
  final response = await http.get(
    Uri.parse(
      '${AppConfig.baseUrl}/pesan_obat/index.php?id_pasien=$idPasien&is_selesai=$isSelesai&_cache=$cacheBuster',
    ),
  );
...
Future pesanObatCreate(PesanObat pesanObat) async {
  ...
  try {
    final response = await http.post(
      Uri.parse('${AppConfig.baseUrl}/pesan_obat/create.php'),
...
Future deletePesanObat(id) async {
  ...
  final response = await http.get(
    Uri.parse('${AppConfig.baseUrl}/pesan_obat/delete.php?id=$id'),
  );
...
Future updatePesanObat(id) async {
  ...
  final response = await http.get(
    Uri.parse('${AppConfig.baseUrl}/pesan_obat/update.php?id=$id'),
  );
